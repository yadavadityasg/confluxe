...
# Install via host cron:  0 3 * * * /opt/wikispace/backup.sh >> /var/log/wikispace-backup.log 2>&1
set -euo pipefail

STACK_DIR="${STACK_DIR:-/opt/wikispace}"
OUT_DIR="${OUT_DIR:-/var/backups/wikispace}"
...
docker run --rm -v wikispace_storage-data:/data -v "$OUT_DIR":/out alpine \
